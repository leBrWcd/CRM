...
<%@ page import="com.lebrwcd.crm.settings.domain.DicValue" %>
<%@ page import="com.lebrwcd.crm.workbench.domain.Tran" %>
<!DOCTYPE html>
<html>
<head>
	<base href="<%=basePath%>">
...
<script type="text/javascript">
	...
	function showHistoryList(){
		...
		$.ajax({
			url : "transaction/showHistoryList.do",
			data : {"tranId" : "${t.id}"},
			dataType : "json",
			type : "get",
	...
	function changeStage(stage,i){
		...
		$.ajax({
			...
			success : function (respData){
				...
				/*respData : {"success" : xxx, "t" : tran对象 */
				if(respData.success){

					//1）改变详细页中 阶段，可能性，修改人，修改时间这四项信息
					$("#stage").html(respData.t.stage);
					$("#possibility").html(respData.t.possibility);
					$("#editBy").html(respData.t.editBy);
					$("#editTime").html(respData.t.editTime);



					//2）改变阶段成功后，将所有的阶段图标重新判断，重新赋予样式及颜色
					changeIcon(stage,i);
	...
	function changeIcon(stage,index){
		...
		if(currentPossibility=="0"){
			...
			//遍历后两个
			for(var i =point;point<<%=dvList.size()%>;i++) {
		...
		//如果当前阶段的可能性不为0，后2个一定是黑叉，前7个可能是绿圈，绿色标记，黑圈
		}else{
		...
		<%
			...
			if("0".equals(currentPossibility)){
				...
				for(int i = 0;i<dvList.size();i++){
					...
					if("0".equals(listPossibility)){
						...
						//大前提，当前阶段的可能性为0，如果遍历出来的阶段是当前阶段，则是红叉
			...
			%>
				<span id="<%=i%>" onclick="changeStage('<%=dv.getValue()%>','<%=i%>')"
					  ...
					  data-content="<%=listStage%>" style="color: red;"></span>
				-----------
			<%
			...
			%>
				<span id="<%=i%>" onclick="changeStage('<%=dv.getValue()%>','<%=i%>')"
					  class="glyphicon glyphicon-remove mystage"
					  data-toggle="popover" data-placement="bottom"
					  data-content="<%=listStage%>" style="color: black;"></span>-----------
			<%

						}


					//如果遍历出来的阶段可能性不为0，说明是前7个
					}else{
			...
			//如果当前阶段的可能性不是0，后2个一定是黑叉，前7个可能是黑圈，绿圈，绿色标记
			}else{
				...
				//大前提，当前阶段的可能性不为0，取出每一个遍历的阶段，取其可能性
				for(int i =0;i<dvList.size();i++) {
					DicValue dv = dvList.get(i);
					String listStage = dv.getValue();
					String listPossibilibty = pMap.get(listStage);

					//如果遍历出来的可能性为0
					if("0".equals(listPossibilibty)){
			...
			<%
						...
						//如果遍历出来的可能性不为0，前7个阶段：绿圈，绿色标记，黑圈

						//如果是当前阶段
						if(i==index){
							//绿色标记
			%>
				<span onclick="changeStage('<%=dv.getValue()%>','<%=i%>')" class="glyphicon glyphicon-map-marker mystage"
					  data-toggle="popover" data-placement="bottom"
